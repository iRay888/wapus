...
LPH_JIT_MAX(function() -- Main Cheat
    ...
    local astar = loadstring(game:HttpGet("https://raw.githubusercontent.com/shlexware/Sirius/request/library/Pathfinding"))() -- fucking flies so it despawns now. ill make pathfinding that stays on the ground. OOOH THAT BROTHA'S FLOATING IN THE AIR
    ...
    --- Calculates the trajectory of an object given its origin, acceleration, target position, and speed.
    --- @param origin Vector The starting position of the object.
    --- @param acceleration Vector The acceleration vector applied to the object.
    ...
    local function trajectory(origin, acceleration, target_position, speed)
        ...
        local displacement_magnitude_squared = 4 * displacement:Dot(displacement)
        local k = (4 * (force:Dot(displacement) + speed * speed)) / (2 * force_magnitude_squared)
        local discriminant = (k * k - displacement_magnitude_squared / force_magnitude_squared) ^ 0.5
        local time1, time2 = k - discriminant, k + discriminant
    
        local time = time1 < 0 and time2 or time1
        time = time ^ 0.5
        return force * time / 2 + displacement / time, time
    end
    

    local solve = debug.getupvalue(physics.timehit, 2)
    --- Classifies the trajectory of an object given its origin, acceleration, target position, speed, and external force.
    --- @param origin Vector The starting position of the object.
    --- @param acceleration Vector The acceleration vector applied to the object.
    --- @param target_position Vector The target position the object aims to reach.
    --- @param speed number The desired speed of the object.
    ...
    local function complexTrajectory(origin, acceleration, target_position, speed, external_force)
        ...
        local root1, root2, root3, root4 = solve(
            acceleration:Dot(acceleration) * 0.25,
            acceleration:Dot(external_force),
            acceleration:Dot(displacement) + external_force:Dot(external_force) - speed^2,
            displacement:Dot(external_force) * 2,
            displacement:Dot(displacement)
        )
    
        local time = (root1 > 0 and root1) or (root2 > 0 and root2) or (root3 > 0 and root3) or root4
        local velocity = (displacement + external_force * time + 0.5 * acceleration * time^2) / time
    ...
    --- Simulates a bullet trajectory given its origin, velocity, and penetration.
    --- @param origin Vector The starting position of the bullet.
    --- @param velocity Vector The velocity vector of the bullet.
    --- @param penetration number The penetration value of the bullet.
    --? Where does bullet traversal angle come from?
    local function simulateBullet(origin, velocity, penetration) -- thank you mickey
        local frames = {}
        local wallHits = {}
        local newTime = 0
    ...
    --- Gets current position of players weapon barrel.
    --- @return Vector The position of the weapon barrel.
...
-- Code in now sexy and if there is anything you want me to do just ask :P